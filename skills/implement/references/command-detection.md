...
1. **Settings override.** A non-empty `cmd_test_unit` / `cmd_test_integration` / `cmd_lint` / `cmd_vet` in `.claude/sdd-emb.local.md` short-circuits everything. This is the escape hatch for unusual repos.
...
5. **Language manifests** (the broad fallback — pick the toolchain the manifest implies):
   ...
   - any other manifest → there is no convention to trust: **ask the user for the commands** (and offer to save them to `.claude/sdd-emb.local.md`) — never guess.
