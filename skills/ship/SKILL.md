...
- `<slug>` — feature slug.
- **Gate (hard refuse):** a `PASS` review record (`docs/features/<slug>/_review/`) or, at minimum, an implemented + gate-green change. No review yet → «спочатку запусти `review <slug>`».
...
6. **Summary (terminal handoff).** **Emit the stage-handoff block** per [`../_shared/handoff.md`](../_shared/handoff.md) (terminal variant) — *Що я зробив* (verification result: verified-working / what was deferred and why; the roadmap update) + *Перевір перед тим як продовжити* (the changelog path + the PR) + *Що далі* = **Готово**: the PR command (or URL if the user ran it) — merging to main is your call; there is no `/sdd-emb` successor.
