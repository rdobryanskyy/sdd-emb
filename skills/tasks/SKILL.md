...
- (Expected) `sad.md` frontmatter `target_surfaces` — gates which layers appear (step 4). **Absent or empty → warn** («поверхні не задекларовано — перезапусти `design`, або продовжуємо як `backend-service`») **and treat as `[backend-service]`** (→ [`../_shared/surfaces.md`](../_shared/surfaces.md)); never silently emit `ui` tasks for an undeclared surface.
...
7. **Per-task DoD.** Each task is testable: «unit tests for the new validation pass», «migration applies and reverts cleanly», «handler returns the spec'd outcome for AC-03». No subjective «done when I say so». If a task's DoD depends on a specific algorithm being correct (e.g. a vectorization, scheduling, or numerical routine), dispatch [`mathematic`](../../agents/mathematic.md) — `subagent_type: "sdd-emb:mathematic"` — directly on that task's slice before writing the DoD, per [`../_shared/math-adversary.md`](../_shared/math-adversary.md); its recommended method becomes part of the task's stated approach so `test-author`/`implementer` build against a reviewed method, not an unreviewed guess.
