...
1. **Check existing.** `test -f docs/features/<slug>/.size` (and `.route`). If `.size` exists, read both values and ask «`.size` зараз `<X>` (маршрут `<Y>`). Перекласифікувати?». On «ні» — STOP (suggest editing, don't overwrite silently). Re-running just to switch the route is a legal, common case — mid-flight override per the Routes table.
...
3. **Map to a class** using the table in [`../_shared/size-matrix.md`](../_shared/size-matrix.md). On an edge case, name the dominant signal aloud («M, бо додає новий API + 1–2 спринти, навіть якщо кількість PR на межі S/M»). For an all-maximums answer, ask explicitly «потрібен окремий roadmap?» → yes = XL.
4. **Confirm size + route — ONE question.** Derive the default route from the size (**XS/S → `quick`, M → `standard`, L/XL → `full`** — the Routes table in [`../_shared/size-matrix.md`](../_shared/size-matrix.md)). Then one `AskUserQuestion`: «Класифікую як `<size>` (<one-line rationale>) → маршрут `<route>` (<one-line what the route does>). Зафіксувати обидва?» — options: `Yes` / `Yes, but route <other>` / `No, I want size <X>` / `Reclassify`. Never a second question just for the route.
