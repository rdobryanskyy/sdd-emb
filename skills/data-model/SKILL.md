...
- `<slug>` — feature slug.
- **Gate (hard refuse if missing):** `docs/features/<slug>/spec.md` (entities live in §5 acceptance criteria) and `docs/features/<slug>/sad.md` (entity candidates: §5 building blocks — the persistence containers — + the §6 persist notes). Missing → «спочатку запусти `specify` / `design`».
...
- (Expected) `sad.md` frontmatter `target_surfaces` — context for which containers persist what. **Absent or empty → warn** («поверхні не задекларовано — перезапусти `design`, або продовжуємо як `backend-service`») **and treat as `[backend-service]`** (→ [`../_shared/surfaces.md`](../_shared/surfaces.md)).
...
7. **Indexes per query.** Each sequence note becomes one index candidate; discard candidates with no concrete query; print a "Query it serves" justification column. If a candidate's shape is a nontrivial partitioning/sharding scheme or a computed/derived column with a real formula (not a plain copy or a simple aggregate), dispatch [`mathematic`](../../agents/mathematic.md) — `subagent_type: "sdd-emb:mathematic"` — directly on that formula before it's written into `data-model.md`, per [`../_shared/math-adversary.md`](../_shared/math-adversary.md).
