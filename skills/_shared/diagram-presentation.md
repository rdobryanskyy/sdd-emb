...
3. **Describe it in prose** — the confirmation prompt is a plain-language account of what the diagram shows, not its source. Name the participants in words and walk the flow as a sentence or two, **including the key branches**. This prose is spoken to the user, so per [`chat-language.md`](./chat-language.md) it's Ukrainian — participant/system names stay as-is, the description around them translates. Example for a sequence flow:
   ...
   For a C4 view: «Контекст показує, як учасник і адміністратор спілкуються з системою Preferences, яка залежить від існуючої системи Identity для перевірки прав і пише в один сховище даних.» Cover every actor/participant and every `alt`/`else` branch in words.
