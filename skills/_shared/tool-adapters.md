...
> name gets an `sdd-emb-` prefix (the bare `review` / `design` / `api` would collide with generic
> names), so the Claude form `/sdd-emb:specify` keeps its mapping: `sdd-emb-specify`.

## The mapping

| Mechanism (as written in the skills) | Claude Code | Codex CLI | Cursor |
|---|---|---|---|
| Invoke a stage | `/sdd-emb:specify <slug>` | `$sdd-emb-specify <slug>` | type `/`, pick `sdd-emb-specify` |
...
| Spawn a subagent (`subagent_type: "sdd-emb:researcher"`) | the named plugin agent | custom agent `sdd-emb-researcher` installed into `.codex/agents/` (dispatch via `/agent`), or run the agent file's instructions inline | subagent `sdd-emb-researcher` installed into `.cursor/agents/`, or inline |
...
| Shared artifacts (`.size`, `.route`, `spec.md` + the other `docs/features/<slug>/…` files, `.claude/sdd-emb.local.md`) | repo-relative files the **model itself** reads/writes with its file tools | identical — no host involvement, so they work unchanged; `.claude/` is just a directory in the repo here, not a host config dir | same as Codex |
