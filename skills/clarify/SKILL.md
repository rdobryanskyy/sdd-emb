...
→ [`../_shared/critic.md`](../_shared/critic.md) (clean-context **dispatch discipline** only — the subagent here hunts AMBIGUITY, not coherence drift) · [`../_shared/ask-style.md`](../_shared/ask-style.md) (resolve/defer question phrasing) · [`../_shared/math-adversary.md`](../_shared/math-adversary.md) (when a clause names or implies a specific algorithm/formula/numeric threshold, dispatch [`mathematic`](../../agents/mathematic.md) as a companion to step 3 — its finding folds into an existing ambiguity class, never a ninth one).
...
- **Gate (hard-refuse if missing):** `docs/features/<slug>/spec.md`. Absent → STOP and point: «спершу запусти `specify <slug>` — clarify загострює вже написану специфікацію, а не пише нову».
...
3. **Devil's-advocate subagent (the core mechanic) + math-adversary companion.** Dispatch the [`devils-advocate`](../../agents/devils-advocate.md) agent — `subagent_type: "sdd-emb:devils-advocate"` (carries its own `model: opus` + `effort: high`; clean context — it never saw this conversation). Pass only the slug + the spec path; it Reads `spec.md` (and `CONTEXT.md`) itself — inline nothing — and returns "two engineers would diverge here" findings. The dispatch follows the contract in [`../_shared/agent-roster.md`](../_shared/agent-roster.md) (clean-isolated context, cited findings, `NO_AMBIGUITIES` if none). If `devils-advocate` is unavailable at runtime, fall back to a `general-purpose` Agent with the prompt body in [`./references/ambiguity-checks.md`](./references/ambiguity-checks.md). If the self-sweep (step 2) or `devils-advocate`'s findings flag a clause naming or implying a specific algorithm/formula/numeric threshold, also dispatch [`mathematic`](../../agents/mathematic.md) — `subagent_type: "sdd-emb:mathematic"` — as a companion pass on that clause (state `math adversary: active` in both prompts, per [`../_shared/math-adversary.md`](../_shared/math-adversary.md)); its finding folds into step 4's merge as the existing class it fits, not a new one.
4. **Merge + dedupe.** Union the self-sweep (step 2) with the subagent findings (step 3); collapse duplicates (same `§ref` + same class). Keep the highest-impact first (conflicting-requirement > under-specified-AC > unmeasured-NFR > undefined-term > missing-actor > scope-creep > vague-term > unstated-assumption). If the merged set is empty → report «специфікація однозначна», stamp nothing, suggest `glossary`/`design`.
...
7. **Stamp + commit.** Set `updated_at: <today>` in the frontmatter. Re-respect `specify`'s invariants when tightening (§5 AC stays free of HTTP/status/error-code/SQL tokens; §6 numbers carry a measurement; §4 roles only from the glossary). Propose `clarify: <slug> — N resolved, M deferred`. Then **emit the stage-handoff block** per [`../_shared/handoff.md`](../_shared/handoff.md) — *Що я зробив* (incl. the terms already reconciled into `CONTEXT.md` by step 6's in-flow `glossary` calls — the user learns here that the glossary changed, not later) + *Перевір перед тим як продовжити* (tightened `spec.md`, + `CONTEXT.md` when terms were added) + *Що далі* — **resolve the next stage per `.route`** (Routes in [`../_shared/size-matrix.md`](../_shared/size-matrix.md)): forward `/sdd-emb:glossary <slug>` ↳ or `/sdd-emb:design <slug>` to skip the glossary hop (on `quick` — terms were already reconciled in-flow, so auto-resolve straight to `/sdd-emb:design <slug>` unless unglossed terms remain; on `full` — keep both options, no auto-resolve).
