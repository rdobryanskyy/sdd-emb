...
14. **Propose commit + handoff.** `adr: <slug> NNNN <title>`. Then **emit the stage-handoff block** per [`../_shared/handoff.md`](../_shared/handoff.md) (utility variant) — *Що я зробив* (incl. «самоперевірка: 6/6 пройдено») + *Перевір перед тим як продовжити* (`adr/NNNN-<title>.md`) + *Що далі*: resume the gate that needed it (`/sdd-emb:tasks <slug>` or `/sdd-emb:plan-tests <slug>`); `/clear` optional.
